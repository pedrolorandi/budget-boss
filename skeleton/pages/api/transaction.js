...
export default async function Adding(req, res) {
  ...
  sources.forEach((item) =>
    item.name === inputSource ? (sourceID = item.id) : false
  ); //record the sourceID when we find it in the sources array
  if (sourceID) {
    //the recorded source exists in the source table
    inputValue.sourceId = sourceID;
    await prisma.transaction.create({ data: inputValue });
    console.log("inputValue", inputValue);
  } else {
    //the recorded source doesn't exist in the source table
